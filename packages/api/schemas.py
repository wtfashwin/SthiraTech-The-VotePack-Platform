...
# --- Commitment Deposit Schemas ---

class CommitmentDepositBase(BaseModel):
    amount: Decimal = Field(..., gt=0, decimal_places=2)
    currency: str = Field("USD", max_length=3)

class CommitmentDepositCreate(CommitmentDepositBase):
    participant_id: uuid.UUID

class CommitmentDepositPublic(CommitmentDepositBase):
    model_config = ConfigDict(from_attributes=True)
...
# --- AI Consensus Schemas ---

class ConsensusProposal(BaseModel):
    """A proposed itinerary/plan generated by the AI consensus engine."""
    title: str
    description: str
    score: float = Field(..., ge=0, le=100, description="Compatibility score (0-100)")
    justification: str
    estimated_budget: Optional[Decimal] = None
    pace: str = Field(..., description="e.g., 'relaxed', 'moderate', 'packed'")
    activities: List[str] = []

class ConsensusProposalsResponse(BaseModel):
    """Response containing multiple AI-generated proposals."""
...
class ImportFromUrlRequest(BaseModel):
    """Request to import activities from a social media URL."""
    url: str = Field(..., min_length=10, max_length=500)

class ImportedActivitySuggestion(BaseModel):
    """An activity suggestion extracted from a URL."""
    title: str
    notes: Optional[str] = None
    location: Optional[str] = None
...
class ImportFromUrlResponse(BaseModel):
    """Response containing suggested activities from a URL."""
    url: str
    suggested_activities: List[ImportedActivitySuggestion]
